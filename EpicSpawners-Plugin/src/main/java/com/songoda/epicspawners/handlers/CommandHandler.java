        ...
        try {
            if (cmd.getName().equalsIgnoreCase("EpicSpawners")) {
                if (args.length == 0 || args[0].equalsIgnoreCase("help") || args[0].equalsIgnoreCase("?")) {
                    ...
                    if (type == null && !args[2].equalsIgnoreCase("random")) {
                        ...
                        if (args[2].equalsIgnoreCase("random")) {
                            ...
                            for (SpawnerData spawnerData : instance.getSpawnerManager().getRegisteredSpawnerData().values()) {
                                if (spawnerData.isActive()) {
                                    list.add(spawnerData.getIdentifyingName().toUpperCase().replace(" ", "_"));
                                }
                            }
                        ...
                        if (args.length == 4) {
                            ...
                            ItemStack spawnerItem = Methods.newSpawnerItem(type, 0, Integer.parseInt(args[3]));
                            ...
                            ItemStack spawnerItem = Methods.newSpawnerItem(type, multi, amt);
                            if (args[1].toLowerCase().equals("all")) {
                                for (Player pl : Bukkit.getOnlinePlayers()) {
                                    pl.getInventory().addItem(spawnerItem);
                                    pl.sendMessage(Arconix.pl().getApi().format().formatText(instance.references.getPrefix() + instance.getLocale().getMessage("command.give.success", amt, Methods.compileName(type, multi, false))));
                ...
                for (Map.Entry<EntityType, Integer> entry : instance.getPlayerActionManager().getPlayerAction(p).getEntityKills().entrySet()) {
                    if (instance.getSpawnerManager().getSpawnerData(Methods.getType(entry.getKey())).getKillGoal() != 0) {
                        size++;
                    }
                }
                ...
                for (Map.Entry<EntityType, Integer> entry : instance.getPlayerActionManager().getPlayerAction(p).getEntityKills().entrySet()) {
                    ...
                    int customGoal = instance.getSpawnerManager().getSpawnerData(type).getKillGoal();
