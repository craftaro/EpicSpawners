            ...
            if (e.getRightClicked() instanceof ItemFrame) {
                if (EpicSpawners.getInstance().dataFile.getConfig().getString("data.entityshop") != null) {
                    ...
                    if (EpicSpawners.getInstance().dataFile.getConfig().getString("data.entityshop." + uuid) != null) {
                        ...
                        EpicSpawners.getInstance().getShop().show(EpicSpawners.getInstance().getSpawnerManager().getSpawnerType(EpicSpawners.getInstance().dataFile.getConfig().getString("data.entityshop." + uuid).toLowerCase()), 1, e.getPlayer());
                    }
                }
            }
        } catch (Exception ex) {
    ...
    public void onPlayerJoin(PlayerJoinEvent e) {
        try {
            ...
            if (p.isOp() && EpicSpawners.getInstance().getConfig().getBoolean("Main.Display Helpful Tips For Operators")) {
                if (EpicSpawners.getInstance().getServer().getPluginManager().getPlugin("Factions") != null && EpicSpawners.getInstance().getHookHandler().FactionsHook == null) {
                    p.sendMessage("");
