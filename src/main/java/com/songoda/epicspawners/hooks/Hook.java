...
public abstract class Hook {
    ...
    protected Hook(String pluginName) {
        ...
        if (isEnabled())
            EpicSpawners.getInstance().getHookHandler().hooksFile.getConfig().addDefault("hooks." + pluginName, true);
    }

    protected boolean isEnabled() {
        return (Bukkit.getPluginManager().isPluginEnabled(pluginName)
                && EpicSpawners.getInstance().getHookHandler().hooksFile.getConfig().getBoolean("hooks." + pluginName, true));
    }
