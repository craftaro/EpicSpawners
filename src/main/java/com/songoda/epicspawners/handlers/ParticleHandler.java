...
public class ParticleHandler {
    ...
    public ParticleHandler(EpicSpawners instance) {
        this.instance = instance;
        try {
            Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(EpicSpawners.getInstance(), this::animate, 0L, 2 * 18);
            Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(EpicSpawners.getInstance(), this::animate2, 0L, 3);
        } catch (Exception e) {
        ...
        for (Spawner spawner : instance.getSpawnerManager().getSpawnersInWorld().values()) { //ToDo: Doesnt work properly for omni spawners.
            ...
            if (spawnerData.getParticleEffect() == ParticleEffect.HALO) {
                ...
                for (int i = 0; i < 360; i++) {
                    ...
                    num = num + 1;
                    Bukkit.getScheduler().scheduleSyncDelayedTask(instance, () -> Arconix.pl().getApi().packetLibrary.getParticleManager().broadcastParticle(loc, 0, 0, 0, 0, spawnerData.getSpawnEffectParticle().getEffect(), spawnerData.getParticleAmount().getEffect()), num);
                }
            }
        }
    }
        ...
        for (Spawner spawner : instance.getSpawnerManager().getSpawnersInWorld().values()) { //ToDo: Doesnt work properly for omni spawners.
            ...
            if (spawnerData.getParticleEffect() == ParticleEffect.TARGET) {
                ...
                for (int i = 0; i < 360; i++) {
                    ...
                    Arconix.pl().getApi().packetLibrary.getParticleManager().broadcastParticle(loc, 0, 0, 0, 0, spawnerData.getSpawnEffectParticle().getEffect(), spawnerData.getParticleAmount().getEffect());
                }
                for (int i = 0; i < 360; i++) {
                    if (i % 10 != 0) continue;
                    double angle = (i * Math.PI / 180);
