...
public class Shop {
    ...
    public void open(Player p, int page) {
        try {
            ...
            for (SpawnerData spawnerData : instance.getSpawnerManager().getSpawnerTypes().values()) {
                if (!spawnerData.getName().toLowerCase().equals("omni")
                        && spawnerData.isInShop()
                        && p.hasPermission("epicspawners.shop." + Methods.getTypeFromString(spawnerData.getName()).replaceAll(" ", "_"))) {
            ...
            for (SpawnerData spawnerData : entities) {
                ...
                ItemStack item = instance.getHeads().addTexture(it, Methods.getTypeFromString(spawnerData.getName()));
            ...
            exitmeta.setDisplayName(instance.getConfig().getString("Interfaces.Exit Icon"));
    ...
    public void show(SpawnerData spawnerData, int amt, Player p) {
        try {
            Inventory i = Bukkit.createInventory(null, 45, Arconix.pl().getApi().format().formatTitle(instance.getLocale().getMessage("interface.shop.spawnershowtitle", Methods.compileName(spawnerData.getName(), 1, false))));
            ...
            p.openInventory(i);
            EpicSpawners.getInstance().inShow.put(p, spawnerData); //ToDo: This system needs to be removed.
        } catch (Exception e) {
            Debugger.runReport(e);
        }
    }
    ...
    public void confirm(Player p, int amt) {
        try {
            SpawnerData spawnerData = EpicSpawners.getInstance().inShow.get(p);
            ...
            ItemStack item = EpicSpawners.getInstance().getApi().newSpawnerItem(spawnerData.getName(), amt);
