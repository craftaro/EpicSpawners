...
public class SpawnOptionEntity implements SpawnOption {
    ...
    public void spawnEntity(Location location, EntityType type, SpawnerData data) {
        try {
            ...
            if (in) {
                ...
                Arconix.pl().getApi().packetLibrary.getParticleManager().broadcastParticle(spot, x, y, z, 0, data.getEntitySpawnParticle().getEffect(), data.getParticleAmount().getEntitySpawn());
    ...
    private boolean spawnFinal(Location location, SpawnerData data, EntityType type) {
        ...
        try {
            ...
            entity.setMetadata("ES", new FixedMetadataValue(instance, data.getName()));

            EpicSpawners.getInstance().getSpawnManager().addUnnaturalSpawn(entity.getUniqueId());
            return true;
        } catch (ReflectiveOperationException e) {
            return false;
        }
